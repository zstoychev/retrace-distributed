...
class Lock {
	...
	bool unlocked;
public:
	Lock(Mutex& mutex) : mutex(mutex), unlocked(false) { mutex.enter(); }
	~Lock() { if (!unlocked) mutex.leave(); }
	void unlock() { mutex.leave(); unlocked = true; }
};
...
struct ThreadData {
	volatile bool alive;
	InterlockedInt* numberActive;
	std::queue<Runnable*>* queuedWorkers;
	Mutex* queuedWorkersMutex;
	Condition* queuedWorkersCondition;
	Condition* waitCondition;
};

int simpleCachedThreadPoolThread(void* data);

template <int N = MAX_CPU_COUNT>
class SimpleCachedThreadPool {
	SDL_Thread* threads[N];
	ThreadData threadsData[N];
	int counter;
	InterlockedInt numberActive;
	std::queue<Runnable*> queuedWorkers;
	Mutex queuedWorkersMutex;
	...
	SimpleCachedThreadPool(const SimpleCachedThreadPool& rhs);
	SimpleCachedThreadPool& operator=(const SimpleCachedThreadPool& rhs);
public:
	SimpleCachedThreadPool()
		: counter(0), numberActive(0), queuedWorkersCondition(queuedWorkersMutex), waitCondition(queuedWorkersMutex) {
		for (int i = 0; i < N; i++) {
			threadsData[i].numberActive = &numberActive;
			threadsData[i].queuedWorkers = &queuedWorkers;
			threadsData[i].queuedWorkersMutex = &queuedWorkersMutex;
			threadsData[i].queuedWorkersCondition = &queuedWorkersCondition;
			threadsData[i].waitCondition = &waitCondition;
		}
	}
	...
	~SimpleCachedThreadPool() {
		...
		{
			Lock lock(queuedWorkersMutex);
			for (int i = 0; i < counter; i++) {
				threadsData[i].alive = false;
			}
		}
		queuedWorkersCondition.signalAll();
	...
	void run_async(Runnable* runnable) {
		{
			Lock lock(queuedWorkersMutex);
			if (counter <= numberActive.get() + (int) queuedWorkers.size() && counter < N) {
				int id = counter++;
				threadsData[id].alive = true;
				threads[id] = SDL_CreateThread(simpleCachedThreadPoolThread, &threadsData[id]);
			}
		
			queuedWorkers.push(runnable);
		}
		queuedWorkersCondition.signal();
	}

	void wait() {
		Lock lock(queuedWorkersMutex);
		while (numberActive.get() != 0 || !queuedWorkers.empty()) {
			waitCondition.wait();
		}
	}
};
