...
struct Triangle {
	int v[3];           //!< Indices in the "triangles" array in the Mesh
	int n[3];           //!< Indices in the "normals" array in the Mesh; zeros, if no normals are provided
	int t[3];           //!< Indices in the "uvs" array in the Mesh; zeros, if no texture coords are provided
	Vector gnormal;     //!< The geometric normal of the triangle
	Vector AB, AC;      //!< B-A and C-A in object space
	Vector ABcrossAC;   //!< AB^AC
	Vector dNdx, dNdy;  //!< dNdx is in object space and corresponds to (1, 0) in texture space. dNdy <-> (0, 1)
	Triangle() {}
...
struct BBox	 {
	...
	inline bool testIntersect(const FastRay& ray) const
	{
	...
	inline double closestIntersection(const FastRay& ray) const
	{
	...
	inline bool intersectTriangle(const Vector& A, const Vector& B, const Vector& C) const
	{
		...
		T.AB = B - A;
		T.AC = C - A;
		T.ABcrossAC = T.AB ^ T.AC;
		double D = A * T.ABcrossAC;
		for (int mask = 0; mask < 7; mask++) {
			for (int j = 0; j < 3; j++) {
				...
				rayEnd[j] = vmax[j];
				if (signOf(ray.start * T.ABcrossAC - D) != signOf(rayEnd * T.ABcrossAC - D)) {
	...
	inline bool intersectWall(Axis axis, double where, const FastRay& ray) const
	{
		if (fabs(ray.dir[axis]) < 1e-9) return (fabs(ray.start[axis] - where) < 1e-9);
		int u = (axis == 0) ? 1 : 0;
		int v = (axis == 2) ? 1 : 2;
		double toGo = where - ray.start[axis];
		const double& rdirInAxis = ray.rdir[axis];
	...
	inline double surfaceArea() const {
		double W = vmax.x - vmin.x;
		double H = vmax.y - vmin.y;
		double L = vmax.z - vmin.z;
