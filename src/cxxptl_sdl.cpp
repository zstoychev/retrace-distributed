...
Condition::Condition(Mutex& mutex)
	: mutex(mutex), condition(SDL_CreateCond()){
}
...
void Condition::wait() {
	SDL_CondWait(condition, mutex.cs);
}

void Condition::wait(Uint32 millis) {
	SDL_CondWaitTimeout(condition, mutex.cs, millis);
}
...
int simpleCachedThreadPoolThread(void* data) {
	...
	while (true) {
		...
		{
			Lock lock(*threadData.queuedWorkersMutex);
			if (!threadData.alive) {
				return 0;
			}
			while (threadData.queuedWorkers->empty()) {
				threadData.queuedWorkersCondition->wait();
				if (!threadData.alive) {
					return 0;
				}
			}

			runnable = threadData.queuedWorkers->front();
			threadData.queuedWorkers->pop();
		
			(*threadData.numberActive)++;
		}

		runnable->run();
		(*threadData.numberActive)--;
		threadData.waitCondition->signal();
	}
